...
## Notes About Compose
- `.env.example` now includes `COMPOSE_PROJECT_NAME=trms` so local Compose commands do not fail with `project name must not be empty`.
...
Recent local validation on `main`:
